...
function shootBullet() {
  ...
  // Visual feedback
  if (playerAvatar) {
    const originalColor = playerAvatar.children[0].material.color.getHex();
    playerAvatar.children[0].material.color.set(0xff6b6b);
    setTimeout(() => {
      playerAvatar.children[0].material.color.set(originalColor);
    }, 100);
  }
}
...
function playerHit() {
  ...
  if (playerAvatar) {
    const originalColor = playerAvatar.children[0].material.color.getHex();
    playerAvatar.children[0].material.color.set(0xff0000);
...
function createOptimizedNFTPlane(nftData, position) {
  // SIMPLIFIED NFT LOADING - smaller textures, faster loading
  const geometry = new THREE.PlaneGeometry(8, 8); // Smaller size
  const textureLoader = new THREE.TextureLoader();
  
  // Use simpler material for faster rendering
  ...
  plane.rotation.x = Math.PI * 0.1; // Slight tilt for better visibility
  ...
  // OPTIMIZED TEXTURE LOADING with error handling and fallback
  const loadNFTTexture = (url) => {
    return new Promise((resolve) => {
      textureLoader.load(url, 
        ...
        undefined,
        (err) => {
          console.warn('Error loading NFT image, using fallback:', err);
          // Use a simple colored fallback
  ...
  const imageUrl = nftData.image_url || 'https://via.placeholder.com/200x200?text=NFT';
  loadNFTTexture(imageUrl);
  ...
  const glowGeometry = new THREE.PlaneGeometry(8.2, 8.2);
  const glowMaterial = new THREE.MeshBasicMaterial({ 
    color: 0x3b82f6,
    transparent: true,
    opacity: 0.3,
...
/* ==============================
   NFT LOADING & MANAGEMENT - IMPROVED
============================== */
...
async function loadNFTs() {
  try {
    ...
    // Use improved NFT distribution
    createWideNFTDistribution(data);
...
function createWideNFTDistribution(nfts) {
  const gridSize = Math.ceil(Math.sqrt(nfts.length));
  const spacing = 120; // WIDER SPACING between NFTs
  const baseHeight = 50;
  ...
  nfts.forEach((nft, index) => {
    ...
    // Create a grid pattern with wider spacing
    const x = (col - gridSize/2) * spacing + (Math.random() * 20 - 10);
    const z = (row - gridSize/2) * spacing + (Math.random() * 20 - 10);
    const y = baseHeight + (Math.random() * 100); // More height variation
    
    // Ensure NFTs stay within world boundaries
    const boundedX = Math.max(-worldBoundary + 100, Math.min(worldBoundary - 100, x));
    const boundedZ = Math.max(-worldBoundary + 100, Math.min(worldBoundary - 100, z));
    
    createNFTPlatform(boundedX, y, boundedZ);
    createOptimizedNFTPlane(nft, { x: boundedX, y: y, z: boundedZ });
  });
}
...
class WebRTCMultiplayer {
  ...
  createOtherPlayer(playerId, playerData) {
    if (window.scene && playerData.position) {
      ...
      // Simplified other player without hover board
      let avatar;
